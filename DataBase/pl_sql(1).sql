...
--------------------------------------------------------------------------------
DECLARE
  v_emprow emp%ROWTYPE;  -- 'v_emprow'는 여기서 선언한 변수이름이고 emp%ROWTYPE 은 emp 객체 자체를 타입으로 쓰겠다는 뜻. 
                         -- 자바로 치면>> EMP v_emprow = new Emp();
BEGIN
  select *
...
DECLARE --declare랑 begin 사이에 변수 이름이랑 타입 쓰는데 여기서는 변수이름 vempno를 지정하고 그것의 타입은 emp.empno의 타입과 같음. 
  vempno emp.empno%TYPE;
  vename emp.ename%TYPE;
  vsal   emp.sal%TYPE;
BEGIN
  select empno , ename , sal
      into vempno , vename , vsal -- into가 변수 이름에 대입한다는 뜻인듯?
  from emp
  where empno=7788;
  --제어문 if(조건문){실행문}
    IF(vsal >=  2000) THEN 
...
--    v_name := CASE vdeptno
    ...
    v_name := CASE  -- ':=' 은 자바의 =이랑 같음 =의 우항의 값을 좌항의 변수이름에 할당하겠다는 의미
                              WHEN vdeptno=10  THEN 'AA'
                              WHEN vdeptno in(20,30)  THEN 'BB' --in(20, 30) 은 20이나 30중에 해당하는거 있는지 , 있으면 'BB'
                              WHEN vdeptno=40  THEN 'CC'
                              ELSE 'NOT'
                      END;
    DBMS_OUTPUT.PUT_LINE('당신의 부서명:' || v_name);      -- '||'은 자바의 + 와 같음 결합 연산자.       
END;
--------------------------------------------------------------------------------
--pl-sql (반복문)
--Basic loop
...
*/
DECLARE 
  n number :=0; -- 변수 'n' 선언했고, 이것의 타입은 number 고, 초기값은 10을 할당하겠다
BEGIN
  LOOP --반복문 (조건이 반복문 Loop 안에 있음)
    DBMS_OUTPUT.PUT_LINE('n value : ' || n);
    n := n + 1;
    EXIT WHEN n > 5; --몇번 반복할건지 (n이 5보다 크면 탈출할거다. 5보다 작으면 계속 반복함)
  END LOOP;
END;
...
DECLARE
  num number := 0;
BEGIN
  WHILE(num < 6) --조건문이 반복문 while  밖에 있음
    LOOP
