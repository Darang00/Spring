...
public class MemberController {
	...
	public String memberUpdate(Model model, Member member, Principal principal){
		...
		updatemember.setPwd(bCryptPasswordEncoder.encode(member.getPwd())); //암호화 해주는 함수 encode
	...
	public String mypageCheck(String pwd, Model model, Principal principal) {
		...
		String url = result ? "redirect:memberupdate.htm" : "mypage.htm"; //3항 연산자 ? 앞에 꺼(result)가 true면 : 앞에 있는 "redirect:memberupdate.htm" 이 뷰페이지 가고 false면 : 뒤에 있는 "mypage.htm" 이게 실행된다. 
		return url;
	}
}
